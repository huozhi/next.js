...
if (process.env.NODE_ENV !== 'production') {
  ;(function () {
    ...
    function describeDifferencesForStylesheetOverPreinit(
      newProps,
      currentProps
    ) {
      var diff = compareResourcePropsForWarning(newProps, currentProps)
    ...
    function describeDifferencesForPreinitOverStylesheet(
      newProps,
      currentProps
    ) {
      var diff = compareResourcePropsForWarning(newProps, currentProps)
        ...
        switch (as) {
          ...
          case 'script': {
...

            {
              ...
              if (_devResource) {
                var _propsEquivalent = scriptPropsFromPreinitOptions(
                  src,
                  options
                )
    ...
    function flushCompletedQueues(request, destination) {
      ...
      beginWriting()
      console.log('beginWriting', performance.now() - start)
    ...
    async function renderToReadableStream(children, options) {
      let startAllBegin = performance.now()
      let ret = new Promise(function (resolve, reject) {
        ...
        function onShellReady() {
          console.log('til shell ready', performance.now() - startAllBegin)
        ...
        console.log('createRequest', performance.now() - start)
        ...
        if (options && options.signal) {
          ...
          console.log('signal handling', performance.now() - start)
        }

        start = performance.now()
        startWork(request)
        console.log('startWork', performance.now() - start)
      })
      await ret
      console.log(
        'server.edge:renderToReadableStream',
        performance.now() - startAllBegin
      )
      return ret
    }
